...
void MMatchServer::OnMatchLogin(const MUID& CommUID, const char* UserID, const char* Password, int CommandVersion, u32 ChecksumPack,
	u32 Major, u32 Minor, u32 Patch, u32 Revision)
{
	...
	if (strcmp(szDBPassword, Password))
	{
	...
	const char* Password, const char* Email)
{
	...
	auto ret = GetDBMgr()->CreateAccountNew(Username, Password, Email);
	switch (ret)
	{
