...
bool MSSQLDatabase::GetLoginInfo(const char* szUserID, unsigned int* poutnAID, char* poutPassword, size_t maxlen)
{
	...
	*poutnAID = rs.Field("AID").AsInt();
	...
	const char* src = rs.Field("PasswordData").AsString().c_str();
	strcpy_safe(poutPassword, maxlen, src);
	if (strlen(src) >= maxlen) {
		mlog("TRUNCAMIENTO: PasswordData original='%s' (len=%zu) copiado='%s' (maxlen=%zu)\n", src, strlen(src), poutPassword, maxlen);
	}
	else {
		mlog("PasswordData copiado correctamente: '%s'\n", poutPassword);
		mlog("TRUNCAMIENTO: PasswordData original='%s' (len=%zu) copiado='%s' (maxlen=%zu)\n", src, strlen(src), poutPassword, maxlen);
	}
...
AccountCreationResult MSSQLDatabase::CreateAccountNew(const char* szUsername,
	const char* szPasswordData, const char* szEmail)
{
	_STATUS_DB_START
		if (!CheckOpen())
	...
	try
	{
		...
		sprintf_safe(sql, "INSERT INTO Login(UserID, AID, PasswordData) VALUES('%s', %d, CONVERT(VARCHAR(50), '%s'))", szUsername, AID, szPasswordData);
		m_DB.ExecuteSQL(sql);
	}
	catch (CDBException* e)
	{
...
bool MSSQLDatabase::InsertConnLog(const int nAID, const char* szIP, const string& strCountryCode3)
{
	...
	{
		mlog("MSSQLDatabase::InsertConnLog - ���ڿ� IP(%s)���� ����. AID(%u)\n",
			nAID, szIP);
		return false;
	}
	...
	const int nBounty, const int nCharBP, const ItemPurchaseType nType)
{
	...
	switch (nType)
	{
	case ItemPurchaseType::Buy:
		strcpy_safe(szType, "����");
		break;
	case ItemPurchaseType::Sell:
		strcpy_safe(szType, "�Ǹ�");
		break;
	}
	...
	const CharMakingType nType)
{
	...
	switch (nType)
	{
	case CharMakingType::Create:
		strcpy_safe(szType, "����");
		break;
	case CharMakingType::Delete:
		strcpy_safe(szType, "����");
		break;
	}
...
bool MSSQLDatabase::GetLadderTeamID(const int nTeamTableIndex, const int* pnMemberCIDArray, const int nMemberCount, int* pnoutTID)
{
	...
	// #ifdef _DEBUG
	strcpy_safe(szFieldName, "TID4"); // ������ ��δ� TID4�� ������
	// #else
...
bool MSSQLDatabase::UpdateQuestItem(int nCID, MQuestItemMap& rfQuestIteMap, MQuestMonsterBible& rfQuestMonster)
{
		...
		try
	{
		...
		{
			assert(0);
			Log("MSSQLDatabase::UpdateQuestItem - �������� ũ�Ⱑ %d���� ŭ.", QUEST_DATA);
			return false;
		}
...
bool MSSQLDatabase::GetCharQuestItemInfo(MMatchCharInfo* pCharInfo)
{
		...
		try
	{
		...
		if (-1 == bn.GetCurIndex())
		{
			mlog("MSSQLDatabase::GetCharQuestItemInfo - ����Ʈ ������ SELECT���� ����. UserName:%s CID:%d\n",
				pCharInfo->m_szName,
				pCharInfo->m_nCID);
			return false;
		}
		...
		for (int i = 0; i < MAX_DB_QUEST_ITEM_SIZE; ++i)
		{
			...
			if (!pCharInfo->m_QuestItemList.CreateQuestItem(nItemID, nQuestItemCount, bKnownItem))
				mlog("MSSQLDatabase::GetCharQuestItemInfo - ���ο� ����Ʈ ������ ���� ����.\n"); // error...
		}
