...
bool RMeshNode::SetBVertData(RBlendVertex* pBVert,int i,int j,int pv_index,int* DifferenceMap,int& matrixIndex)
{
	...
	if( pPhysique->m_num > 3 || pPhysique->m_num <= 0 ) {
		mlog("%s mesh %s node %d face %d point -> physique 3 �� �̻�\n",m_pParentMesh->GetFileName() ,m_Name.c_str(),i,j);
		return false;
	}
...
void RMeshNode::CalcVertexBuffer_Physique(const rmatrix& world_mat, int frame)
{
	...
	// MEJORA: Caché de matrices transformadas por bone ID para evitar recalcular
	// Usamos un array estático local para cachear matrices usadas en este frame
	// Máximo de bones únicos que pueden aparecer
	...
	// Resetear caché al inicio del cálculo
	cachedCount = 0;

	// MEJORA: Buffers estáticos para reutilizar memoria entre frames
	static rvector* cachedPositions = NULL;
	static int cachedPositionsSize = 0;
	
	// Redimensionar buffer estático si es necesario
	...
	for(i=0;i<m_physique_num;i++) { // point_num
		...
		for(j=0;j<p_num;j++) {// 4
			...
			// MEJORA: Buscar en caché primero antes de calcular
			...
			// Si no está en caché, calcular y agregar (mantener lógica original)
			if (pCachedMat == NULL && cachedCount < MAX_CACHED_BONES)
			{
				t_mat = pTMP->m_mat_result;  // Mantener lógica original
			...
			{
				// Caché lleno, calcular directamente
