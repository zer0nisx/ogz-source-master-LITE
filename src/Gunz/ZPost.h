...
void ZPostLogin(const char* szUserID, char* szPassword, unsigned int ChecksumPack);
...
inline void ZPostCreateAccount(const char* Username, const char* HashPassword, const char* Email)
{
	ZPOSTCMD3(MC_MATCH_REQUEST_CREATE_ACCOUNT, MCmdParamStr(Username), MCmdParamStr(HashPassword), MCmdParamStr(Email));
}
