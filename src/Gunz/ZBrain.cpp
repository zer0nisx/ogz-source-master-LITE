...
// REFACTORIZACIÓN: Helper para verificar si una tarea bloquea el uso de skills
...
{
	// REFACTORIZACIÓN: Usar helper en lugar de código duplicado
...
// REFACTORIZACIÓN: Helper para verificar si una tarea bloquea el pathfinding
...
bool ZBrain::CheckSkillUsable(int *pnSkill, MUID *puidTarget, rvector *pTargetPosition)
{
	if(m_pBody->GetNPCInfo()->nNPCAttackTypes & NPC_ATTACK_MAGIC) {
		...
		// OPTIMIZACIÓN: Distancia máxima para considerar un objetivo aliado
		...
		for(int i=0;i<pmod->GetSkillCount();i++) {
			...
			if(pDesc->IsAlliedTarget()) {	// ����� �Ʊ��ΰ��
				...
				// OPTIMIZACIÓN: Pick es costoso, solo hacerlo si IsUsable pasó
...
bool ZBrain::FindTarget()
{
	...
	// OPTIMIZACIÓN: Distancia máxima para considerar un objetivo (evitar buscar objetivos muy lejanos)
		...
		itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		// OPTIMIZACIÓN: Early exits para mejor rendimiento
		if (pCharacter->IsDead()) continue;
		
		if (pCharacter->LostConnection())
			continue;

		// OPTIMIZACIÓN: Calcular distancia primero (más barato que CheckEnableTargetting)
		...
		{
			// Guardar como objetivo temporal si está más cerca
			if (distSq < fDist)
			{
		...
		// OPTIMIZACIÓN: Ya calculamos distSq arriba, reutilizar
		if (distSq < fDist)
		{
			fDist = distSq;
	...
	// CORRECCIÓN: Si no encontramos objetivo válido pero hay uno temporal, usarlo
